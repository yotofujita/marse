#!/usr/bin/env python3
"""Compute SI-SDR for each sample and write values to metrics.csv."""
...
def main() -> None:
    ...
    for idx, sample_dir in enumerate(tqdm(iter_sample_dirs(args.test_exp_dir), desc="sisdr computation")):
        if args.max_items is not None and idx >= int(args.max_items):
            break
        try:
            enhanced, clean, _ = load_aligned_pair(sample_dir)
            score = compute_sisdr(enhanced, clean)
            update_metrics_csv(sample_dir, {"SI_SDR": float(score)})
